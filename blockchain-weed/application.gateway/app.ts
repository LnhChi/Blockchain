...
async function startExpressServer(contract: Contract, gateway: Gateway, client: grpc.Client): Promise<void> {
    ...
    app.post('/api/weed-detection', async (req, res) => {
        try {
            const payload = req.body;
            if (!payload || !payload.scan_id) {
                res.status(400).json({ error: 'Missing scan_id in payload' });
                return;
            }
            ...
            console.log(`Submitting RecordWeedDetection to Blockchain for id: ${id}`);
            
            // Chỉ lưu lên Blockchain
            const resultBytes = await contract.submitTransaction('RecordWeedDetection', id, payloadStr);
            const txId = Buffer.from(resultBytes).toString('utf8');
            console.log(`Transaction committed for id: ${id}, txId: ${txId}`);
            
            res.status(201).json({ 
                success: true, 
                message: 'Record saved to Blockchain',
                txId: txId
            });
        } catch (error) {
            console.error('Error in /api/weed-detection:', error);
            res.status(500).json({ error: String(error) });
        }
    });

    app.get('/api/weed-history/:id', async (req, res) => {
        try {
            const id = req.params.id;
            console.log(`Evaluating GetWeedHistory from Blockchain for id: ${id}`);
            const resultBytes = await contract.evaluateTransaction('GetWeedHistory', id);
            const resultJson = Buffer.from(resultBytes).toString('utf8');
            res.status(200).json(JSON.parse(resultJson));
        } catch (error) {
            console.error('Error in /api/weed-history:', error);
            res.status(500).json({ error: String(error) });
        }
    });
